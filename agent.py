...
class CACLA(Agent):
    ...
    def _normal(self, output):
        ...
        noise = self.noise * torch.tensor((self._action_high - self._action_low) / 2).to('cuda')
        self.writer.add_scalar("sigma", noise)

        return Normal(output, noise)
    ...
    def update_actor_ac(self, targets, values):
        ...
        # compute losses
        # log_prob = self._distribution.log_prob(self._action)
    ...
    def update_actor_cacla(self, targets, values):
        advantages = targets - values.detach()
